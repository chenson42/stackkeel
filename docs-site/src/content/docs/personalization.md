---
title: Personalize Your Next.js AI Starter Kit
description: "How Stackkeel's guided personalization turns a template copy into your project: an identity interview, app and feature selection, module stripping with rollback, brand generation from one seed color, and a manifest that powers upstream sync."
---
...
| `UNPERSONALIZED` | Origin points somewhere else but the identity is still the kit's, so personalization is required |
...
10. Writes `kit.json`: `role: "fork"`, your module selection, the personalized/removed path records that the sync skills rely on. The diff is left uncommitted for your review.
